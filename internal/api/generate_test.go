...
func TestGenerateRejectsBadJSON(t *testing.T) {
	...
	for _, body := range []string{
		`{"analysisId":`,
		`{"analysisId":"a","requestId":"r","target":"curl","extra":true}`,
		`{"analysisId":"a","requestId":"r","target":"curl"}{}`,
		strings.Repeat("x", generateBodyLimit+1),
	} {
		w := httptest.NewRecorder()
		mux.ServeHTTP(w, httptest.NewRequest(http.MethodPost, "/api/v1/generate", strings.NewReader(body)))
		if w.Code != http.StatusBadRequest {
